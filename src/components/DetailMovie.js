...
function DetailMovie() {
  ...
  useEffect(() => {
    /* al usar el Backend se hara una validacion de disponibilidad del estado de la pelicula elegida, si hay stock o no para su alquiler */
  ...
  return (
      <div className="parent-detail-movie h-full">
        ...
        <div className="ml-6 mt-10 div2-detail-movie">
          <h1 className='text-6xl font-title text-white text-left-i'>{objMovie.name}</h1>
          <h1 className='mt-3 text-2xl font-title text-gray text-left-i'>{objMovie.year} | {objMovie.time} | {objMovie.clasification}</h1>
        </div>
        <div className="div3-detail-movie mt-10 text-right mr-6">
          <h1 className='text-5xl font-title text-white'>{objMovie.rating} <span className='text-orange'>★</span> </h1>
          { validateMovieStatus ? 
          <button
              type="button"
        ...
        <div className="ml-6 mr-6 div4-detail-movie text-left-i">
          <h1 className='text-1xl font-title text-white text-justify mb-3'>{objMovie.resume}</h1>
          <span className='text-1xl font-title text-gray text-justify'>Director:</span>  <span className='text-1xl font-title text-white text-justify'>{objMovie.director}</span><br />
          <span className='text-1xl font-title text-gray text-justify'>Género:</span>  <span className='text-1xl font-title text-white text-justify'>{objMovie.gender.toString()}</span>
        </div>
